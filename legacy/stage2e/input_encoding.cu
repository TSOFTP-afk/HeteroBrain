...
// =============================================================================
// 与字节模式并存 (向后兼容): 新增 int32 BPE token 注入路径
//   - 数据源: prepare_bpe_data.py 输出的 wikitext2_bpe.bin (int32 token 流)
//   - token_id 范围: 0..50256 (distilgpt2 vocab_size=50257)
//   - 游标 g_bpe_pos 独立于字节模式游标 g_text_pos
//   - 注入: token_id % N_COLUMNS_2E 选偏好柱, 偏好柱 gain_in, 其余 gain_out
// 下方所有函数为新增, 不修改上方字节模式函数
// =============================================================================

// 全局 BPE token 流缓冲 (host 端, 独立于 g_text_buffer)
static std::vector<int32_t> g_bpe_buffer;
static size_t g_bpe_pos = 0;
static bool g_bpe_loaded = false;
static uint64_t g_bpe_fingerprint = 0;

// distilgpt2 vocab size (BPE 回退模式用, 与 prepare_bpe_data.py 一致)
static constexpr int32_t BPE_VOCAB_SIZE = 50257;

// 加载 BPE token 流 (.bin, int32 little-endian 数组) 到全局缓冲
// 返回: 加载的 token 数 (0 表示失败)
size_t load_bpe_stream(const char* filepath) {
    std::ifstream ifs(filepath, std::ios::binary);
    if (!ifs) {
        std::cerr << "[BPEStream] 无法打开文件: " << filepath << std::endl;
        return 0;
    }

    // 读取整个文件为原始字节, 再重解释为 int32 数组
    std::string raw((std::istreambuf_iterator<char>(ifs)),
                     std::istreambuf_iterator<char>());

    if (raw.empty()) {
        std::cerr << "[BPEStream] 文件为空: " << filepath << std::endl;
        g_bpe_buffer.clear();
        g_bpe_pos = 0;
        g_bpe_loaded = false;
        g_bpe_fingerprint = 0;
    ...
    // 对齐校验: 字节数必须是 4 (sizeof int32) 的整数倍
    if (raw.size() % sizeof(int32_t) != 0) {
        std::cerr << "[BPEStream] 文件大小 " << raw.size()
                  << " 不是 4 字节对齐, 可能已损坏: " << filepath << std::endl;
        g_bpe_buffer.clear();
        g_bpe_pos = 0;
        g_bpe_loaded = false;
        g_bpe_fingerprint = 0;
    ...
    size_t n_tokens = raw.size() / sizeof(int32_t);
    g_bpe_buffer.resize(n_tokens);
    std::memcpy(g_bpe_buffer.data(), raw.data(), raw.size());

    g_bpe_pos = 0;
    g_bpe_loaded = !g_bpe_buffer.empty();

    // FNV-1a 指纹 (与 text_corpus_fingerprint 同算法, 作用于原始字节)
    uint64_t fingerprint = 1469598103934665603ULL;
    for (unsigned char c : raw) {
        fingerprint ^= c;
        fingerprint *= 1099511628211ULL;
    }
    g_bpe_fingerprint = fingerprint;

    // 诊断输出 (镜像 load_text_corpus 风格)
    std::cout << "[BPEStream] 加载 " << filepath << ": "
              << raw.size() << " 字节 -> " << n_tokens << " tokens" << std::endl;
    std::cout << "  FNV-1a 指纹: 0x" << std::hex << g_bpe_fingerprint << std::dec << std::endl;
    std::cout << "  token 预览 (前 10):";
    for (size_t i = 0; i < 10 && i < g_bpe_buffer.size(); ++i) {
        std::cout << " " << g_bpe_buffer[i];
    }
    std::cout << std::endl;

    return g_bpe_buffer.size();
}

// 检查 BPE 流是否已加载
bool is_bpe_loaded() { return g_bpe_loaded; }

// 获取已加载 BPE 流的 token 数
size_t bpe_stream_size() { return g_bpe_buffer.size(); }

// 获取 BPE 流指定位置的 token (越界返回 0)
int32_t get_bpe_token_at(size_t idx) {
    if (idx >= g_bpe_buffer.size()) return 0;
    return g_bpe_buffer[idx];
}

size_t bpe_stream_position() { return g_bpe_pos; }

bool set_bpe_stream_position(size_t position) {
    if (!g_bpe_loaded || g_bpe_buffer.empty()) return position == 0;
    g_bpe_pos = position % g_bpe_buffer.size();
    return true;
}

uint64_t bpe_stream_fingerprint() { return g_bpe_fingerprint; }

// =============================================================================
// inject_bpe_token_kernel: BPE token 群体编码注入
// =============================================================================
// 镜像 input_inject_kernel, 但用 token_id % N_COLUMNS_2E 选偏好柱
// (字节模式用 byte 落在柱偏好字节范围 [col*range, (col+1)*range) 内判定)
// 每 thread 处理一个柱, 在柱内 L4 层激活 K 个神经元
// 哈希: xorshift32, 种子 = token * 2654435761 + col_seed (与字节模式同构)
// =============================================================================
__global__ void inject_bpe_token_kernel(
    float* __restrict__ input_current,
    int32_t token,
    int n_columns,
    float gain_in,         // 偏好柱增益倍数 (COLUMN_BYTE_PREF_GAIN_IN = 2.0)
    float gain_out,        // 非偏好柱增益倍数 (COLUMN_BYTE_PREF_GAIN_OUT = 0.03)
    const ThalamicGateState* gate_states)  // 丘脑门控状态 (每柱一个, nullptr 时全开)
{
    int col = blockIdx.x * blockDim.x + threadIdx.x;
    if (col >= n_columns) return;

    // token_id % n_columns 选择偏好柱 (单柱偏好, 替代字节模式的多字节范围)
    // 用 uint32 取模避免负 token (防御性, 正常 token ∈ [0, 50256])
    uint32_t tok_u = static_cast<uint32_t>(token);
    int preferred_col = static_cast<int>(tok_u % static_cast<uint32_t>(n_columns));
    bool is_preferred = (col == preferred_col);
    float gain = is_preferred ? gain_in : gain_out;

    // 丘脑门控调制: gate_signal ∈ [0,1] 缩放增益
    // 向后兼容: nullptr 时全开 (gate=1.0)
    float gate = (gate_states != nullptr) ? gate_states[col].gate_signal : 1.0f;

    // 柱内 L4 层基址 (丘脑输入层, 位于柱首 200 神经元, 索引计算与字节模式一致)
    int col_base = col * NEURONS_PER_COLUMN_2E;
    int sensory_base = col_base;  // 0..199 是 L4 (丘脑输入层)

    // 柱特定的哈希种子 (Knuth 乘法常数, 与字节模式一致)
    uint32_t col_seed = static_cast<uint32_t>(col) * 0x9E3779B9u;
    uint32_t hash = tok_u * 2654435761u + col_seed;

    // 在柱内 L4 层 (COL_L4_SIZE_2E 神经元) 中激活 K 个
    for (int k = 0; k < POP_CODING_K_PER_COLUMN; ++k) {
        // xorshift32 哈希
        hash ^= hash << 13;
        hash ^= hash >> 17;
...
// Host launcher (BPE)
// =============================================================================
// 镜像 launch_input_inject, 但接受 int32_t token 而非 uint8_t byte
// d_gate_states: 丘脑门控状态数组 (设备指针), nullptr 时全开 (向后兼容)
void launch_bpe_inject(MemoryAllocator* alloc, int32_t token,
    ...
    inject_bpe_token_kernel<<<1, threads>>>(
        b.d_input_current,
        token,
        N_COLUMNS_2E,
        COLUMN_BYTE_PREF_GAIN_IN,       // 2.0f (复用字节模式偏好增益)
        COLUMN_BYTE_PREF_GAIN_OUT,      // 0.03f (复用字节模式非偏好增益)
        d_gate_states);
}
...
// 计算当前步应注入的 BPE token
// =============================================================================
// 真实模式: 从 BPE 流循环读取 int32 token (每调用一次推进游标, 与 get_byte_for_step 同构)
// 回退模式: 未加载时使用 step % 50257 循环 (distilgpt2 vocab size)
int32_t get_token_for_step(int step) {
    if (g_bpe_loaded && !g_bpe_buffer.empty()) {
        // 真实 BPE 模式: 从 token 流循环读取
        int32_t tok = g_bpe_buffer[g_bpe_pos];
        g_bpe_pos = (g_bpe_pos + 1) % g_bpe_buffer.size();
        return tok;
    }
