...
// 启动配置: <<<ceil(N/256), 256>>>
//   - 每线程一神经元 i
//   - 对 256 维 error 做点积: Σ_b W[i*256+b] · error[b]
//   - W_decode 行主序 (N×256), warp 内 32 个连续 b 读 128B = 1 cache line
//
// 量级分析 (验证 ELIGIBILITY_GAIN 选择):
//   - |error| ≈ 1/256 ≈ 0.004 (均匀分布假设)
//   - |W_decode[i*256+b]| 归一化后 ≤ 1/√256 ≈ 0.0625
//   - 单元素乘积 ≈ 0.00025, 256 求和后 ≈ 0.064
//   - 加上 gain=0.01 → 增量 ≈ 6.4e-4
//   - 衰减因子 λ=exp(-1/20)≈0.95, 稳态 ≈ 增量/(1-λ) ≈ 0.013
//   - 与 e1 (稳态≈0.3) 乘积 ≈ 4e-3, 与原 delta_w≈0.03 同量级 (符合预期)
// =============================================================================
__global__ void decode_eligibility_update_kernel(
    float* __restrict__ neuron_eligibility,
    const float* __restrict__ decode_weights,
    const float* __restrict__ error,
    int n_neurons,
    float decay_factor,
    float gain)
{
    ...
    // 1. 点积 backprop_signal = Σ_b W_decode[i*256+b] · error[b]
    //    W_decode 行主序: 行 i 起始地址 = i * 256
    const float* row = decode_weights + (size_t)i * 256;
    float backprop_signal = 0.0f;
    #pragma unroll 4
    for (int b = 0; b < 256; ++b) {
        backprop_signal += row[b] * error[b];
    }

    // 2. 衰减 + 增量更新 (一阶低通滤波)
    //    符号约定: backprop_signal = Σ W·error 表示"神经元对预测误差的责任" (positive=有责)
    //    转换为"信用": credit = -blame, 这样 neuron_eligibility > 0 = 应该强化,
    //    neuron_eligibility < 0 = 应该削弱。STDP kernel 中 e1·neuron_eligibility 即得正确方向。
    //    neuron_eligibility[i] = λ · old - g · backprop_signal
    float old_val = neuron_eligibility[i];
    float new_val = decay_factor * old_val - gain * backprop_signal;

    // 3. 数值稳定: clamp 到 [-1, 1] 避免极端情况下漂移
    //    (理论稳态 ≈ 0.013, clamp 不会影响正常训练)
...
{
    if (!buf.d_neuron_eligibility || !buf.d_decode_weights
        || !buf.d_decode_error) {
        // 防御: 缓冲未分配时跳过 (与 decode_step 一致的防御模式)
        return;
    }

    int blocks = (N_TOTAL_NEURONS_2E + THREADS_PER_BLOCK_2E - 1) / THREADS_PER_BLOCK_2E;
    // 衰减因子 λ = exp(-dt/τ), dt=1 步, τ=NEURON_ELIB_TAU ms
    float decay = expf(-1.0f / NEURON_ELIB_TAU);

    decode_eligibility_update_kernel<<<blocks, THREADS_PER_BLOCK_2E>>>(
        buf.d_neuron_eligibility,
        buf.d_decode_weights,
        buf.d_decode_error,
        N_TOTAL_NEURONS_2E,
        decay,
        DECODE_ELIGIBILITY_GAIN);
    CUDA_CHECK_LAST_2E();
}
