...
void BioMechanismScheduler::step(int current_step) {
    ...
    // Task D3: BPE 模式下 (bptt_input_mode==1) 由 main.cpp 在 step() 之前调用
    //          launch_bpe_inject, 此处跳过字节注入避免双重注入
    //          字节模式 (bptt_input_mode==0) 仍由 scheduler 内部调用 launch_input_inject
    if (is_inject_step && !is_sleeping_ && bptt_input_mode == 0) {
    ...
    // ==================== Task D2: BPTT 代理梯度训练 ====================
    // 在所有现有 kernel (含 STDP + decode_step) 之后执行
    // 每窗口 (bptt_window_size 步) 触发一次完整 forward+backward+update
    if (bptt_active() && !is_sleeping_) {
        bptt_step(current_step, current_byte, bptt_last_target_token_);
    }
...
void BioMechanismScheduler::launch_structural_plasticity(int step) {
    ...
    //
    // Task F6: BPTT 模式下跳过阶段 2 (结构重建)
    //   原因: BPTT trainer 在 forward/backward 中直接使用 buf.d_synapses 与
    //         buf.d_csr_row_ptr, P3-D 结构重建会修改这两个数组 (prune 突触后重建 CSR),
    //         导致 BPTT 用新 CSR 索引访问基于旧拓扑的 V/S history, 触发 GPU 死锁.
...
void BioMechanismScheduler::decode_step(uint8_t current_input_byte,
    ...
    if (is_inject_step && update_weights) {
        ...
        if (input_byte_history_count_ >= PREDICTION_DELAY_STEPS) {
            ...
            // Task F3.1: 双轨累积 (窗口 + 全局)
            cross_entropy_loss_accum_ += h_loss;
            loss_accum_count_++;
            cross_entropy_loss_global_ += h_loss;   // 全局累积, 不被日志重置
            loss_global_count_++;
    ...
    if (decode_step_counter_ % PERPLEXITY_LOG_INTERVAL == 0 && loss_accum_count_ > 0) {
        ...
        // Task F3.1: 仅重置窗口累积器 (全局累积器 cross_entropy_loss_global_ 保留供 final_perplexity 使用)
...
// Task D2: BPTT 集成实现
// =============================================================================
// init_bptt: 分配 BPTTTrainer (懒分配), 由 main.cpp 在启用 BPTT 模式时调用一次
// shutdown_bptt: 释放 BPTTTrainer, 由析构函数自动调用
// bptt_step: 主 step() 末尾调用, 累加计数器, 窗口满时触发 forward+backward+update
// =============================================================================

bool BioMechanismScheduler::init_bptt(const BPTTConfig& config) {
    if (bptt_trainer_ != nullptr) {
        shutdown_bptt();  // 已存在, 先释放
    }
    bptt_window_size = config.window_size;
    bptt_trainer_ = new BPTTTrainer(config, N_TOTAL_NEURONS_2E, N_TOTAL_SYNAPSES_2E);
    bptt_step_counter_ = 0;
    bptt_last_loss_ = 0.0f;
    bptt_last_grad_norm_ = 0.0f;
    bptt_current_lr_ = 0.0f;
    std::printf("[BPTT] 初始化完成: window=%d, lr=%.4f, clip=%.1f, warmup=%d, alpha=%.2f\n",
                config.window_size, config.lr, config.grad_clip,
                config.warmup_steps, config.surrogate_alpha);
    return true;
}

void BioMechanismScheduler::shutdown_bptt() {
    if (bptt_trainer_ != nullptr) {
        delete bptt_trainer_;
        bptt_trainer_ = nullptr;
    }
    bptt_step_counter_ = 0;
}

void BioMechanismScheduler::bptt_step(int current_step, uint8_t current_byte, int32_t current_token) {
    if (!bptt_active()) return;

    bptt_step_counter_++;

    // 窗口未满, 等待
    if (bptt_step_counter_ < bptt_window_size) return;

    // 窗口满, 触发 forward + backward + update
    PersistentBuffers& buf = alloc_->buffers();
