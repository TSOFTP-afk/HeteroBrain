    ...
    float eta_multiplier)
{
    ...
    //   Phase 3a 扩充: 6 维调质向量
    //   M_ij = σ(da_rec·DA + ach_rec·ACh + ne_rec·NE + ht5_rec·5HT
    //             - gaba_rec·GABA           // GABA 抑制可塑性门控 (抗焦虑保护)
    //             + oxy_rec·Oxytocin)       // 催产素增强社交相关可塑性 (共情驱动)
    ...
    if (g_e0_ablation) {
        ...
        M_ij = 1.0f / (1.0f + expf(-(s.da_receptor * da_conc[post]
                                     ...
                                     - get_gaba_receptor(s) * gaba_conc[post]
                                     + oxy_rec * oxytocin_conc[post])));
    ...
    if (effective_signal > 0.0f) {
        float evidence = eta_alpha * effective_signal * plasticity_gain * M_ij * plasticity_factor;
        synapse_alpha[i] += evidence;
    } else if (effective_signal < 0.0f) {
        float evidence = eta_beta * (-effective_signal) * plasticity_gain * M_ij * plasticity_factor;
    ...
    float eta_multiplier)
{
    ...
    // P2: 三因素调制 (E0 消融模式下跳过)
    // Phase 3a: 6 维调质门控 (与 stdp_dual_trace_kernel 一致)
    float M_ij, plasticity_factor;
    if (g_e0_ablation) {
        M_ij = 1.0f;
        plasticity_factor = 1.0f;
