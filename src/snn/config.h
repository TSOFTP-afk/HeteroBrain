...
// -----------------------------------------------------------------------------
// 全局日志静默开关 (2026-08-07)
// 引擎 (vita_engine) 启动时调用 set_silent_mode(true): 隐藏 SNN 内部诊断日志
// (显存分配表 / 调度器初始化 / 文本加载统计等), 仅保留关键与错误信息。
// snn_train 训练工具保持默认 false (完整日志)。
// 定义在 input_encoding.cu (唯一的符号定义点)。
// -----------------------------------------------------------------------------
namespace stage2e {
extern bool g_silent_mode;
extern void set_silent_mode(bool on);
}
...
#define PSW_ETA_ALPHA             200.0f
#define PSW_ETA_BETA              200.0f
...
// -----------------------------------------------------------------------------
// eligibility 闭环 (闭环修复: 突触级 + 神经元级 + 预测误差反传)
// -----------------------------------------------------------------------------
// 设计意图:
//   原 STDP kernel 仅使用瞬时 delta_w 累积 PSW 证据, eligibility trace (e1/e2)
//   计算后从未被权重更新读取 (死代码)。修复让权重更新使用 trace 替代瞬时 delta_w,
//   引入神经元级 eligibility (由解码器预测误差反传驱动), 实现三因素学习闭环:
//     evidence = η · (e1·neuron_eligibility[post]) · M_ij · plasticity_factor
//   - e1: 突触级快 trace (短期 STDP 历史)
//   - neuron_eligibility[post]: 由 decode error × W_decode^T 反传的神经元级信号
//   - M_ij: 神经调质门控 (DA/ACh/NE/5HT)
//   - plasticity_factor: CaMKII 巩固调制
//
// 增益参数 (相对量级控制):
